...
class Card:  # pylint: disable=too-few-public-methods
    ...
    def real_value(self):
        """returns the real value of the card, if the card is a Jack, Queen or King, it returns 10"""
        if self.value > 10:
            return 10
        return self.value
...
class Hand:
    ...
    def __str__(self):
        string = (
            f"Number of cards : {self.nb_cards} ,total = {self.get_value()} ,cards = "
        )
        if self.nb_cards == 0:
            return string + "None"
...
class Deck:
    ...
    def __str__(self):
        return f"Number of cards [DECK] : {self.nb_cards} , cards = {self.l_cards}"
